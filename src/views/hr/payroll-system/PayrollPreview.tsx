...
import type {
  MappedEmployee,
} from "../../../views/hr/payroll-system/mapPayrollVerificationData";
...
const StatChip: React.FC<{
  ...
  <div className="bg-card border border-theme rounded-xl p-3 flex items-center gap-3 min-w-0">
    ...
    <div className="min-w-0 flex-1">
      ...
      <p className={`text-sm font-extrabold tabular-nums leading-tight truncate ${valueClass}`}>
        {value}
      </p>
                  ...
                  <div className="space-y-2">
                    {emp.ctc > 0 && (
                      <div className="flex justify-between text-[12px]">
                        ...
                        <span className="font-semibold text-main tabular-nums">{fmtMoney(emp.ctc, currency)}</span>
                      </div>
                    )}
                    {emp.annualTaxable > 0 && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-muted">Annual taxable amount</span>
                        <span className="font-semibold text-main tabular-nums">{fmtMoney(emp.annualTaxable, currency)}</span>
                      </div>
                    )}
                    {emp.currentMonthTax > 0 && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-muted">Income tax this month</span>
                        <span className="font-semibold text-danger tabular-nums">{fmtMoney(emp.currentMonthTax, currency)}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {emp.earnings.length > 0 && (
                <div className="px-5 py-4 border-b border-theme">
                  ...
                  {emp.earnings.map((e, i) => (
                    <div key={e.abbr || i} className="flex justify-between items-center py-2 border-b border-theme last:border-0">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {!!e.is_flexible_benefit && <span className="text-[9px] font-semibold text-blue-500 bg-blue-50 dark:bg-blue-900/20 rounded px-1">Flex</span>}
                        {!e.is_tax_applicable && <span className="text-[9px] font-semibold text-muted bg-app border border-theme rounded px-1">Non-tax</span>}
                      </div>
                      <span className="text-[12px] font-semibold text-main tabular-nums ml-4 shrink-0">{fmtMoney(e.amount ?? 0, currency)}</span>
                    </div>
                  ))}
                  <div className="flex justify-between items-center mt-2 pt-2 border-t-2 border-theme">
                    ...
                    <span className="text-[13px] font-extrabold text-main tabular-nums">{fmtMoney(emp.gross, currency)}</span>
                  </div>
                </div>
              )}
              ...
              {emp.deductions.length > 0 && (
                <div className="px-5 py-4 border-b border-theme">
                  ...
                  {emp.deductions.map((d, i) => (
                    <div key={d.abbr || i} className="flex justify-between items-center py-2 border-b border-theme last:border-0">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {!!d.variable_based_on_taxable_salary && <span className="text-[9px] font-semibold text-amber-600 bg-amber-50 dark:bg-amber-900/20 rounded px-1">Variable</span>}
                      </div>
                      <span className="text-[12px] font-semibold text-danger tabular-nums ml-4 shrink-0">−{fmtMoney(d.amount ?? 0, currency)}</span>
                    </div>
                  ))}
                  <div className="flex justify-between items-center mt-2 pt-2 border-t-2 border-theme">
                    <span className="text-[11px] font-extrabold text-danger">Total deductions</span>
                    <span className="text-[13px] font-extrabold text-danger tabular-nums">−{fmtMoney(emp.totalDeductions, currency)}</span>
                  </div>
                </div>
              )}
              ...
              <div className="px-5 py-4">
                <div className="rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 p-4 flex items-center justify-between">
                  <div>
                    ...
                    {emp.totalInWords && (
                      <p className="text-[10px] text-emerald-700/70 dark:text-emerald-400/70 leading-snug max-w-[220px]">{emp.totalInWords}</p>
                    )}
                  </div>
...
export const PayrollPreviewModal: React.FC<PayrollPreviewModalProps> = ({
  ...
  const employeeCount   = rawData?.financial_summary?.employee_count
  ...
  const handleSortChange = ({ sortBy: key, sortOrder: order }: { sortBy: string; sortOrder: "asc" | "desc" }) => {
    setSortBy(key);
    setSortOrder(order);
  };
  ...
  const exportExcel = useCallback(() => {
    ...
    for (const emp of employees) {
      for (const e of emp.earnings)   { if (e.abbr) earningAbbrSet.set(e.abbr, e.salary_component || e.abbr); }
      for (const d of emp.deductions) { if (d.abbr) deductionAbbrSet.set(d.abbr, d.salary_component || d.abbr); }
    }
    ...
    const totalRow = [
      "", "TOTALS", "", "", "", "", "",
      ...earningCols.map(([abbr]) => employees.reduce((s, e) => s + (e.components[abbr] ?? 0), 0)),
      rawData.financial_summary?.total_gross_payable ?? 0,
      ...deductionCols.map(([abbr]) => employees.reduce((s, e) => s + (e.components[abbr] ?? 0), 0)),
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3 pb-2 relative">
          ...
          <ModalTable<MappedEmployee>
            ...
            enableExport={false}      // using our own export button above
