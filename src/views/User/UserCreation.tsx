...
const CreateUserPage: React.FC = () => {
  ...
  useEffect(() => { setPage(1); }, [searchQuery]); // ← added

  // ── fetchUsers takes no params — reads state directly ────────────────────
  const fetchUsers = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getUsers(searchQuery, page, pageSize);
      if (response.status !== "success") throw new Error(response.message ?? "Failed to fetch users");
      setUsers(response.data);
      setTotalItems(response.pagination.total);
      setTotalPages(response.pagination.total_pages);
  ...
  }, [page, pageSize, searchQuery]); // ← dependencies
  ...
  useEffect(() => {
    const unsubscribe = useDataRefreshStore 
         .getState()
      .subscribeToRefresh(REFRESH_KEYS.CREATE_USER_LIST, () => {
        fetchUsers();
      });
    return unsubscribe;
  }, []);
  ...
  const handleAdd = () => {
    openUserModal(null, false, {
      onSuccess: fetchUsers, // ← no args needed now
      onSubmit: async (data: unknown) => {
        await createUser(data as CreateUserFormData);
      },
    });
  ...
  return (
    <div className="h-full min-h-0">
      <Table
        loading={loading || initialLoad} // ← added initialLoad
        columns={columns}
        data={users}
        rowKey={(row) => row.id}
        tableId="create-user-management"
        ...
        showToolbar
        searchValue={searchQuery}
        onSearch={(q) => { setSearchQuery(q); setPage(1); }} // ← simplified
        enableColumnSelector
        enableAdd={can(USER_MODULE, "create")}
        addLabel="Add User"
        onAdd={handleAdd}
        ...
        totalItems={totalItems}
        pageSizeOptions={[10, 25, 50, 100]}
        onPageSizeChange={(size) => { setPageSize(size); setPage(1); }} // ← simplified
        onPageChange={setPage} // ← simplified, no redundant fetchUsers call
      />
    </div>
  );
};
