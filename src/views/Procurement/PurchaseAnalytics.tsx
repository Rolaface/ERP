  ...
  const sections = [
    {
      ...
      label: isQuantity ? "Purchase Quantity" : "Purchase Value",
      items: [
        {
          label: "Total",
          value: fmtVal(kpis?.total_purchase_value || 0),
...
const PurchaseAnalytics: React.FC = () => {
    ...
    async (currentFilters: PurchaseAnalyticsFilters) => {
      ...
      try {
        ...
        if (res?.status === "success" || res?.message?.status_code === 200) {
          ...
          setError(res?.message?.message || "Failed to load Purchase Analytics.");
        }
      } catch (err: unknown) {
        setError(
  ...
  const treeTypeOptions: PurchaseAnalyticsFilters["tree_type"][] = [
    "Supplier",
    "Supplier Group",
    "Item",
  ...
  const rangeOptions: PurchaseAnalyticsFilters["range"][] = [
    "Weekly",
    "Monthly",
    "Quarterly",
    "Yearly",
    ...
    <div className="h-full min-h-0 flex flex-col w-full p-2 gap-3">
      ...
      >
        ...
        <FilterDropdown
          ...
          active={filters.tree_type !== "Supplier"}
