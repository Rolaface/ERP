...
const InvoiceList: React.FC<InvoiceListProps> = ({ form, onFormChange }) => {
  ...
  if (!isSupported) {
    return <EmptyMessage text={`Invoice allocation is not available for party type "${partyType}".`} />;
  }
  if (loading) {
    return (
  ...
  if (fetchError) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3 text-red-500">
        ...
        <button onClick={retryFetch} className="text-xs text-primary underline hover:opacity-80">
          Retry
        </button>
      </div>
    );
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="rounded-xl border border-[var(--border)] overflow-hidden">
        ...
        <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr_1fr_32px] bg-[var(--row-hover)] border-b border-[var(--border)] px-4 py-2.5">
          {(["Invoice No", "Due Date", "Total", "Paid", "Outstanding", "Allocate"] as const).map((h, i) => (
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-0.5">
                  <span className="text-xs font-medium text-main">{inv.invoiceNumber}</span>
                ...
                <div className="flex justify-end">
                  {isEditing ? (
                    <input
                      ...
                      onChange={(e) => handleInputChange(inv.invoiceNumber, e.target.value)}
                      onBlur={() => handleInputBlur(inv.invoiceNumber, inv.outstanding)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") handleInputBlur(inv.invoiceNumber, inv.outstanding);
