...
const Detail: React.FC<DetailProps> = ({
  ...
  const viewValue =
    name === "dateAdded" && !isEditing
      ? isoToDisplay(String(safe)) || String(safe)
      : String(safe);

  const displayValue =
    !isEditing && masked && !revealed && !isEmpty ? maskValue(safe) : viewValue;
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div
        className={`border rounded-xl px-4 py-3 flex items-center justify-between gap-3 transition-all duration-150 bg-card ${
          isEditing ? "border-primary ring-2 ring-primary/15" : "border-theme"
        }`}
      >
        {isEditing ? (
          name === "dateAdded" ? (
...
const BankDetails: React.FC<Props> = ({
  ...
  const isDefaultSelected =
    normalizedAccounts[selectedAccount ?? -1]?.isdefault;
        ...
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="col-span-2 bg-card rounded-2xl shadow-sm border border-theme overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              <div
                className={`space-y-2.5 ${isEditing ? "opacity-50 pointer-events-none" : ""}`}
              >
                  ...
                  filteredAccounts.map((acc, i) => {
                    ...
                    return (
                      <div
                        key={acc.id ?? i}
                        onClick={() => {
                          if (!isEditing && globalIdx !== -1)
                            setSelectedAccount(globalIdx);
                        }}
                        className={`p-3 rounded-xl border cursor-pointer transition-all duration-150 ${
                          isSelected
                            ? "border-primary/40 bg-primary/5 shadow-sm"
                            : "border-theme bg-card hover:border-primary/25"
                        } ${isEditing ? "cursor-not-allowed opacity-60" : ""}`}
                      >
                        <div className="flex justify-between items-start mb-0.5">
                          <div className="flex items-center gap-2 min-w-0">
                            <p
                              className={`font-semibold text-sm truncate ${isSelected ? "text-primary" : "text-main"}`}
                            >
                              {acc.bankName}
                            </p>
                            {acc.isdefault && (
                          ...
                          <p className="text-[11px] text-muted shrink-0 ml-2 font-mono">
                            {acc.swiftCode}
                          </p>
                        </div>
            ...
            !bankAccounts[selectedAccount]?.bankName?.trim() ? (
              <div className="flex flex-col items-center justify-center p-16 text-center">
                ...
                <p className="text-sm text-muted">
                  Select an account from the list to view its details
                </p>
              </div>
            ) : (
              <div className="p-6">
