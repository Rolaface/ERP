...
// ─── Empty placeholder item (matches CreditNoteItem shape) ───────────────────
// Used to pre-populate the table before an invoice is selected,
// so ItemTable always renders the same row structure.
...
const PLACEHOLDER_COUNT = 1; // same as Invoice modal (1 blank row initially)
...
export const CreditNoteDetailsTab: React.FC<CreditNoteDetailsTabProps> = ({
  ...
  // ── Decide what to render in the table ────────────────────────────────────
  //
  //  • No invoice selected  → PLACEHOLDER_COUNT empty rows (disabled inputs)
  //  • Invoice loading      → PLACEHOLDER_COUNT pulsing rows (same structure)
  //  • Items loaded         → real rows
  //
  ...
  const actions = {
    addItem: () => {},         // no "Add Item" needed for credit notes
    duplicateItem: () => {},   // no duplicate
