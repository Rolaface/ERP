...
import { Search, Package, ChevronDown, X, AlertTriangle, Loader2 } from "lucide-react";
...
function ExpiryCell({ date }: { date?: string }) {
  ...
  return <span className="text-[11px] text-main whitespace-nowrap">{label}</span>;
}
...
function StockCell({ qty, isService }: { qty: number; isService?: boolean }) {
  ...
  const isOut = qty <= 0;
  return (
    <span className={`tabular-nums font-semibold text-[11px] ${isOut ? "text-danger" : "text-success"}`}>
      {qty}
    </span>
  );
}
...
function FilterPill({
  ...
  value: StockFilter; active: boolean; onClick: () => void;
}) {
  const labels: Record<StockFilter, string> = { all: "All", instock: "In Stock", expired: "Expired" };
  return (
    <button
      type="button"
      onClick={onClick}
...
export default function StockItemSelect({
        ...
        if (item.batches?.length) {
          item.batches.forEach((b: any) => {
            rows.push({
              ...
              warehouse:      b.warehouse,        // kept in data, hidden from UI
              qty:            b.bal_qty,
  ...
  const handleSelect = (row: any) => {
    ...
    onChange({
      ...
      warehouse:      row.warehouse,              // passed through, not shown
  ...
  const filtered = useMemo(() => {
    ...
    return flatRows
      .filter((r) => {
        const hay = `${r.itemName ?? ""} ${r.itemCode ?? ""} ${r.batchNo ?? ""}`.toLowerCase();
        if (tokens.length && !tokens.every((t) => hay.includes(t))) return false;
        if (stockFilter === "instock") return r.isServiceItem || Number(r.qty ?? 0) > 0;
        if (stockFilter === "expired") return getExpiryStatus(r.expiryDate) === "expired";
        return true;
      })
      .sort((a, b) => Number(b.qty ?? 0) - Number(a.qty ?? 0));
  }, [flatRows, search, stockFilter]);

  // ── dropdown position: smart flip ─────────────────────────────────────────
  // Opens BELOW trigger by default.
  // If space below < DROP_H and more space is available above → flips UP.
  ...
  const dropStyle = useMemo((): React.CSSProperties => {
    ...
    const PADDING = 12;
    const MAX_W   = 680;
    // toolbar ~40px + thead ~32px + 4 rows×40px + footer ~28px = ~260px
    const DROP_H  = 260;
    const vw      = window.innerWidth;
    const vh      = window.innerHeight;

    const width = vw <= 640 ? vw - PADDING * 2 : Math.min(MAX_W, vw - PADDING * 2);
  ...
  return (
    <div className="w-full">
      ...
      <div
        ...
        onClick={handleOpen}
        className={[
          "flex items-center gap-2 px-2.5 py-1.5 border border-theme rounded-md",
          "bg-card text-main text-[11px] cursor-pointer select-none transition-colors duration-150",
          disabled ? "opacity-50 pointer-events-none" : "hover:border-primary focus:outline-none focus:border-primary",
        ].join(" ")}
      >
        <Package className="w-3.5 h-3.5 text-muted shrink-0" />
        <span className={`flex-1 truncate ${displayName ? "text-main" : "text-muted"}`}>
      ...
      {open && dropRect && createPortal(
        ...
        >
          ...
          {/* ── TOOLBAR: filter pills + search in ONE compact row ── */}
          <div className="flex items-center gap-2 px-3 py-2 border-b border-theme bg-app">

            {/* filter pills — always visible, never hidden */}
            <div className="flex items-center gap-1 shrink-0">
              {(["all", "instock", "expired"] as StockFilter[]).map((f) => (
                <FilterPill key={f} value={f} active={stockFilter === f} onClick={() => setStockFilter(f)} />
              ))}
            </div>
            ...
            {stockFilter !== "all" && (
              <span className="inline-flex items-center gap-1 text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium shrink-0">
                ...
                <button type="button" onClick={() => setStockFilter("all")} className="hover:opacity-70">
          ...
          {/* ── TABLE ── */}
          {/*
            Height budget (shows exactly 4 rows without scroll):
              thead:  32px (sticky)
              4 rows: 4 × 40px = 160px
              total:  192px
            Cap to 45vh so it never overflows on small / zoomed screens.
          */}
          <div className="overflow-y-auto" style={{ maxHeight: "min(192px, 45vh)" }}>
            {loading ? (
              ...
              /*
               * table-fixed + colgroup percentages:
               *   Item/Code 38% — wraps long names naturally inside cell
               *   Batch     20% — truncates with title tooltip
               *   MFG       15% — date string, always fits
               *   EXP       17% — coloured date, always fits
               *   Stock     10% — right-aligned number
               * No pixel widths anywhere → scales to any container width.
               */
                ...
                <thead className="sticky top-0 z-10 bg-app">
                  <tr className="border-b border-theme">
                    {[
                      ...
                      <th key={label} className={`px-3 py-2 ${cls} text-[10px] font-semibold text-muted tracking-wide uppercase`}>
                        {label}
                      </th>
                    ))}
                  </tr>
                  ...
                  {filtered.map((row) => {
                    ...
                    return (
                      <tr
                        ...
                        role="option"
                        aria-selected={false}
                        onClick={() => { if (!isDisabled && !loading) handleSelect(row); }}
                        className={[
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 align-middle">
                          <p className="font-medium text-main text-[11px] leading-snug break-words">{row.itemName}</p>
                          <p className="text-[10px] text-muted mt-0.5 truncate">{row.itemCode}</p>
                        </td>

                        {/* Batch */}
                        <td className="px-3 py-2.5 align-middle">
                          <span className="block truncate text-[11px] text-main" title={row.batchNo || "-"}>
                            {row.batchNo || "-"}
                          </span>
                        </td>
                        ...
                        <td className="px-3 py-2.5 align-middle text-right">
                          <StockCell qty={qty} isService={row.isServiceItem} />
                        </td>
                      </tr>
                    );
                  })}
          ...
          <div className="flex items-center justify-between px-3 py-1.5 border-t border-theme bg-app text-[10px] text-muted">
            <span>
              ...
              {flatRows.length !== filtered.length && ` of ${flatRows.length}`}
            </span>
            {(stockFilter !== "all" || search) && (
              <button
                type="button"
                className="text-primary hover:underline font-medium"
                onClick={() => { setStockFilter("all"); setSearch(""); }}
              >
                Clear filters
              </button>
            )}
