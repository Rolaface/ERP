...
export const generatePurchaseOrderPDF = async (
  ...
  const drawWatermark = () => {
    if (company?.documents?.companyLogoUrl) {
      try {
        ...
        doc.addImage(px(company.documents.companyLogoUrl), "PNG", (W - 80) / 2, H / 2 - 40, 80, 80);
  ...
  // Logo
  const LOGO_X = M, LOGO_Y = 5, LOGO_SZ = 38;
  if (company?.documents?.companyLogoUrl) {
    try {
      doc.addImage(px(company.documents.companyLogoUrl), "PNG", LOGO_X, LOGO_Y, LOGO_SZ, LOGO_SZ);
    } catch { /* ignore */ }
  } else {
  ...
  stripCols.forEach(([label, val], i) => {
    const sx = i * scw + 5;
    doc.setFont("helvetica", "normal"); doc.setFontSize(5.5); doc.setTextColor(180, 210, 255);
    doc.text(label, sx, SY + 5);
    doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.setTextColor(...WHITE);
    doc.text(val, sx, SY + 13);
    if (i > 0) {
      doc.setDrawColor(100, 140, 200); doc.setLineWidth(0.3);
  ...
  const calcBoxH = (lines: string[], hasBoldTop = false) => {
    ...
    lines.forEach(l => { h += doc.splitTextToSize(l, colW - 6).length * LH; });
    return h + 2;
  };
  const boxH = Math.max(calcBoxH(supplierL, true), calcBoxH(dispatchL), calcBoxH(shippingL));
  ...
  if (company?.documents?.authorizedSignatureUrl) {
    try {
      doc.addImage(px(company.documents.authorizedSignatureUrl), "PNG",
        M + (SIG_W - 50) / 2, SEC_Y + 10, 50, 18);
    } catch { /* ignore */ }
  } else {
    doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); doc.setTextColor(...INK_PALE);
  ...
  autoTable(doc, {
    ...
    styles: { fontSize: 8, cellPadding: { top: 2.5, bottom: 2.5, left: 5, right: 5 }, lineColor: RULE, lineWidth: 0.15 },
    columnStyles: {
      0: { fontStyle: "bold", fillColor: TINT,  cellWidth: SUM_W * 0.52, textColor: INK_SOFT },
      1: { halign: "right",   fillColor: WHITE, cellWidth: SUM_W * 0.48, textColor: INK_SOFT },
    },
    didParseCell: (d) => {
    ...
    margin: { left: SUM_X, right: M }, tableWidth: SUM_W,
  });
  ...
  if (termsY + tBH > H - 16) { doc.addPage(); drawWatermark(); termsY = 16; }
