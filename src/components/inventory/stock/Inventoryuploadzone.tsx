// ─── Inventory Import — Bulk Upload Zone ─────────────────────────────────────
...
import type { BulkRow } from "../../../types/Stockcorrection.types";
...
const SHEET_COLS = [
  ...
  { key: "itemCode", label: "Item Code", width: 140, align: "left" },
  { key: "itemName", label: "Item Name", width: 200, align: "left" },
  { key: "itemGroup", label: "Item Group", width: 140, align: "left" },
  { key: "uom", label: "UOM", width: 90, align: "center" },
  { key: "warehouse", label: "Warehouse", width: 180, align: "left" },
  { key: "openingQty", label: "Opening Qty", width: 110, align: "right" },
  { key: "valuationRate", label: "Valuation Rate", width: 130, align: "right" },
  { key: "brand", label: "Brand", width: 130, align: "left" },
      ...
      >
                  ...
                  return (
                    <tr
                      ...
                      onMouseEnter={(e) => {
                        ...
                        ).style.background = isErr
                          ? "rgba(220,38,38,0.07)"
                          : "rgba(201,125,46,0.04)";
                      }}
                      onMouseLeave={(e) => {
                        (
                          e.currentTarget as HTMLTableRowElement
                    ...
                    >
                      <td
                        style={{
                          ...
                          color: "var(--muted,#bbb)",
                          fontWeight: 600,
                          borderBottom: "1px solid var(--border,#f0ece4)",
                          borderRight: "1px solid var(--border,#f0ece4)",
                        }}
                      >
                        {rows.indexOf(row) + 1}
                      </td>
        ...
        >
          <span style={{ fontSize: 11, color: "var(--muted,#999)" }}>
            Showing{" "}
            <strong style={{ color: "var(--main,#2d2d2d)" }}>
              {filtered.length}
            </strong>{" "}
            of{" "}
            <strong style={{ color: "var(--main,#2d2d2d)" }}>
  ...
  <div className="flex items-center gap-4">
    {STEPS.map((step, i) => (
      <React.Fragment key={step.n}>
        ...
        {i < STEPS.length - 1 && (
          <div className="flex-1 h-px bg-theme min-w-[12px]" />
        )}
      </React.Fragment>
    ))}
  </div>
    ...
    <div className="flex items-start gap-3">
      ...
      <div>
        ...
        <p className="text-xs text-muted mt-0.5">
          Columns: item_code · item_name · item_group · uom · warehouse ·
          opening_qty · valuation_rate · description · brand
        </p>
      </div>
    </div>
    <button
...
const BulkPreviewRow: React.FC<{ row: BulkRow; index: number }> = ({
  row,
  index,
}) => (
  <tr
  ...
  >
    ...
    <td className="px-3 py-2.5 text-main">{row.itemName || "—"}</td>
    <td className="px-3 py-2.5 text-xs text-muted">{row.itemGroup || "—"}</td>
    <td className="px-3 py-2.5 text-xs text-muted">{row.uom || "—"}</td>
    <td className="px-3 py-2.5 text-xs text-muted">{row.warehouse || "—"}</td>
    <td className="px-3 py-2.5 tabular-nums font-medium text-main">
      {row.openingQty}
    </td>
    <td className="px-3 py-2.5 tabular-nums font-medium text-main">
      {row.valuationRate}
