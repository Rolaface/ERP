...
import { modalFallback, isRecord, toQuickAddText } from "../../components/modals/modalHelpers";
import type { ModalCallback, ModalSubmitHandler } from "../../types/modal";
    ...
    if (pending) {
      ...
      if (modalType) {
        useModalStore.getState().openModal(modalType, null, false, {
          source: "quickAdd",
          fieldId: pending.fieldId,
         callback: pending.callback as unknown as ModalCallback,
          onSuccess: (data) => {
            if (!isRecord(data)) return;
            completeQuickAdd({
              id: toQuickAddText(data.id || data.customerId),
  ...
  return (
    <>
      {modals.map((modal) => {
        ...
        const renderFn = modalRegistry[modal.type];
        if (!renderFn) return null;

        return (
          <React.Fragment key={modal.id}>
            <Suspense fallback={modalFallback}>
              {renderFn(modal, context, { handleClose, handleSubmit })}
            </Suspense>
          </React.Fragment>
        );
      })}
