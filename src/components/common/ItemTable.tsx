...
const InvoiceHeaders: React.FC = () => (
  <tr className="border-b border-theme">
    ...
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[130px] whitespace-nowrap">Item</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[140px] whitespace-nowrap">Description</th>
    ...
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[130px] whitespace-nowrap">Box</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[250px] whitespace-nowrap">Batch No</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[50px] whitespace-nowrap">Qty</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] whitespace-nowrap">Mfg Date</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[50px] whitespace-nowrap">Expiry Date</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[100px] whitespace-nowrap">Warehouse</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[40px] whitespace-nowrap">
      Unit Price <span className="text-danger">*</span>
    </th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[60px] whitespace-nowrap">Dis(%)</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[70px] whitespace-nowrap">Tax(%)</th>
    <th className="px-2 py-1 text-left text-muted font-medium text-[11px] w-[60px] whitespace-nowrap">Tax Code</th>
      ...
      <tr key={i} className="border-b border-theme bg-card row-hover">
        ...
        <td className="px-0.5 py-1 min-w-[135px]">
          <StockItemSelect
            ...
            onClear={() =>
              actions.updateItemDirectly?.(i, {
                itemCode: "", description: "", batchNo: "",
                mfgDate: "", expDate: "", packingUnit: "",
                packingSize: "", price: 0, vatRate: undefined, vatCode: "",
              })
            }
          />
        </td>
        ...
        <td className="px-0.5 py-1">
          <div className="flex items-center gap-1">
            <Tooltip content={`Unit: ${it.packingUnit}`}>
              <input type="number" name="packingUnit" value={it.packingUnit || ""} disabled
                onChange={(e) => actions.handleItemChange(i, e)}
            ...
            <Tooltip content={`Size: ${it.packingSize}`}>
              <input type="number" name="packingSize" value={it.packingSize || ""} disabled
                onChange={(e) => actions.handleItemChange(i, e)}
                className="w-[38px] py-1 px-1 border border-theme rounded text-[10px] bg-card text-main text-center no-spinner"
              />
            </Tooltip>
        ...
        <td className="px-0.5 py-1">
          <div className="flex items-center gap-1">
            <Tooltip content={`Box Start: ${it.boxStart}`}>
              <input name="boxStart" value={it.boxStart || ""} placeholder="Start"
                onChange={(e) => actions.handleItemChange(i, e)}
            ...
            <Tooltip content={`Box End: ${it.boxEnd}`}>
              <input name="boxEnd" value={it.boxEnd || ""} placeholder="End"
                onChange={(e) => actions.handleItemChange(i, e)}
                className="w-[38px] py-1 px-1 border border-theme rounded text-[10px] bg-card text-main"
              />
            </Tooltip>
        ...
        <td className="px-0.5 py-1 min-w-[100px]">
          <Tooltip content={`Batch No: ${it.batchNo}`}>
            <input type="text" name="batchNo" value={it.batchNo} disabled
              onChange={(e) => actions.handleItemChange(i, e)}
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (qty > available - usedQty) {
                  showValidationError(`Only ${available - usedQty} items remaining in batch ${it.batchNo}`);
                  return;
                }
        ...
        <td className="px-0.5 py-1">
          <Tooltip content={it.warehouse || "No warehouse selected"}>
            <WarehouseSelect compact value={it.warehouse || ""}
              onChange={(e) =>
                actions.handleItemChange(i, { target: { name: "warehouse", value: e.target?.value ?? e } } as any)
              }
            />
          </Tooltip>
        </td>
        ...
        <td className="px-0.5 py-1">
          <Tooltip content={`Price: ${symbol} ${Number(it.price || 0).toFixed(2)}`}>
            <input type="number" name="price" value={it.price ?? ""}
        ...
        {/* Discount */}
        <td className="px-0.5 py-1">
          <input type="number" name="discount" value={it.discount ?? ""}
        ...
        {/* VAT Rate */}
        <td className="px-0.5 py-1">
          <input type="number" name="vatRate" value={it.vatRate ?? ""}
            className="w-[55px] py-1 px-2 border border-theme rounded text-[11px] bg-card text-main focus:outline-none focus:ring-1 focus:ring-primary no-spinner"
            onChange={(e) => actions.handleItemChange(i, e)}
          />
        </td>
        ...
        <td className="px-0.5 py-1">
          <div className="flex items-center gap-1">
            <Tooltip content="Duplicate row below">
              <button type="button" onClick={() => handleCopyRow(i)}
                className="p-0.5 rounded bg-primary/10 text-primary hover:bg-primary/20 transition"
              >
                <Copy className="w-4 h-4" />
              </button>
            </Tooltip>
            <button type="button" onClick={() => handleRemoveRow(i)}
    ...
    <div className="bg-card rounded-lg p-1 shadow-sm">
      ...
      <div className="mt-2 overflow-x-auto">
        <table className="w-full min-w-[760px] border-collapse text-[10px] leading-tight">
          <thead>
            {columnHeaders ?? <InvoiceHeaders />}
          </thead>
          <tbody>
            {paginatedItems.map((it, idx) => {
              const i = ui.page * ITEMS_PER_PAGE + idx;
              return renderRow
                ? <React.Fragment key={i}>{renderRow(it, i, { handleCopyRow, handleRemoveRow })}</React.Fragment>
      ...
      <div className="flex justify-between mt-3">
        <button
          type="button" onClick={actions.addItem}
        ...
        {(ui.itemCount > ITEMS_PER_PAGE || ui.page > 0) && (
          <div className="flex items-center gap-3 py-1 px-2 bg-app rounded">
            <div className="text-[11px] text-muted whitespace-nowrap">
              ...
              {Math.min((ui.page + 1) * ITEMS_PER_PAGE, ui.itemCount)} of {ui.itemCount} items
            </div>
