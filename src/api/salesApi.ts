...
export async function getSalesInvoiceById(
  ...
  is_sales_debit_note: boolean = false
): Promise<any | null> {
  const url = `${InvoiceAPI.getById}?id=${encodeURIComponent(id)}&is_credit_note=${is_credit_note}&is_sales_debit_note=${is_sales_debit_note}`;
