...
import type { UploadedAttachment } from "../api/Email/EmailApi";
...
export function useSendEmail({
    ...
    const handleSend = useCallback(async () => {
        ...
        if (to.length === 0) {
            showValidationError("Please add at least one recipient.");
            return;
        }
        ...
        if (!invoiceNumber) {
            showValidationError("Document number is missing.");
            return;
        }
